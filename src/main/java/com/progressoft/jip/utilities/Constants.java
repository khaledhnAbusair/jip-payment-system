...
public class Constants {
	...
	public static final String UPDATE_PAYMENT_PURPOSE_SQL_STATEMENT = "UPDATE " + DB_NAME + "." + PAYMENT_PURPOSE_TABLE
			+ " SET NAME = ? WHERE CODE = ?";
	public static final String LOAD_PAYMENT_REQUEST_BY_ID_SQL_STATEMENT = "select ID as id,ORD_IBAN as orderingAccountIBAN,BENEF_IBAN as beneficiaryAccountIBAN,BENEF_NAME as beneficiaryName,AMOUNT as paymentAmount,CURRENCY_CODE as currencyCode,PURPOSE_CODE as purposeCode,PAYMENT_DATE as paymentDate from "
			+ Constants.DB_NAME + "." + Constants.PAYMENT_REQUEST_TABLE_NAME + " where id = ?";
	...
	public static final String SELECT_CRNCY_BY_CODE = MessageFormat.format(
			"select CODE as currencyCode, NAME as currencyDescription, COINS_NAME as coinsName from {0} where {1} = ?",
			CRNCY_TABLE_NAME, CRNCY_CODE_COLOMN);
	public static final String SELECT_ALL_CRNCYS = MessageFormat.format(
			"select CODE as currencyCode,NAME as currencyDescription,COINS_NAME as coinsName  from {0}",
			CRNCY_TABLE_NAME);
	...
	public static final String INSERT_ACCOUNT = "insert into PAYMENT_SYSTEM.ACCOUNT values(?,?,?,?,?,?)";
