...
import static com.progressoft.jip.gateway.impl.MySqlCurrencyGateway.*;
import static com.progressoft.jip.gateway.impl.MySqlCurrencyGateway.CRNCY_TABLE_NAME;
...
public class MySQLCurrencyDBMock {
	...
	public static void tearDownDBEnviroment(BasicDataSource dataSource) {
		String sql = "TRUNCATE " + CRNCY_TABLE_NAME;
		try (Connection connection = dataSource.getConnection()) {
	...
	private static void insertCurrencyIntoTable(Connection connection, String Code, String Name, double rate)
			throws SQLException {
		String sql = "INSERT INTO " + CRNCY_TABLE_NAME + " VALUES(?,?,?)";
	...
	public static double selectUpdatedCurrencyRateValue(BasicDataSource dataSource) {
		try {
			...
			resultSet.next();
			return resultSet.getDouble(CRNCY_RATE_COLOMN);
		} catch (SQLException e) {
			throw new IllegalStateException(e);
		}
	}
